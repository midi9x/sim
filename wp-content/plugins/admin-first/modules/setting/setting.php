...
class Admin_Theme_Setting{
	...
	function use_default_style($style) {
		$file__menu = $this->plugin_path.'/scss/scss/_variables_menu_'.$style.'.scss';
		$file__bar = $this->plugin_path.'/scss/scss/_variables_bar_white.scss';
		if(file_exists($file__menu)){
		    file_put_contents($this->plugin_path.$this->file_menu, file_get_contents($file__menu), FILE_TEXT );
		}
		if(file_exists($file__bar)){
		    file_put_contents($this->plugin_path.$this->file_bar, file_get_contents($file__bar), FILE_TEXT );
		}
	}
