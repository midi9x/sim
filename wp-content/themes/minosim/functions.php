...
function sim_post_type() {
    ...
    $args = array(
        ...
        'taxonomies'            => array( 'sim_category', 'sim_tag', 'network' ),
        'hierarchical'          => false,
        'public'                => true,
        'show_ui'               => true,
        'show_in_menu'          => true,
...
function thong_tin_sim_html( $post) {
    ...
    <p>
        ...
        <input type="text" name="price" id="price" value="<?php echo thong_tin_sim_get_meta( 'price' ); ?>">
    </p><?php
}
...
function add_submenu() {
    ...
    add_submenu_page( 'edit.php?post_type=sim', 'Xóa sim', 'Xóa sim', 'manage_options', 'update_sim', 'update_sim' );
}
function add_sim() {
?>
        ...
        <table class="widefat" style="border-radius:4px;">
            <tbody>
                ...
                <tr class="alternate iedit">
                    ...
                    <td colspan="2" style="padding:20px 12px;">
                        <label><input type="checkbox" name="update" id="update" value="1" checked> Cập nhật số đã có trên website</label>
                    </td>
                </tr>
                <tr>
                    <td></td>
    ...
    <script type="text/javascript">
        jQuery(document).ready(function($) {
            ...
            jQuery('#formAddSim').on('submit', function(event) {
                ...
                var update = $('#update').is(":checked") ? 1 : '';
                jQuery.ajax({
                    type: 'POST',
                    url: ajaxurl,
                    data: { 
                    ...
                    success: function(data) {
                        ...
                        if (data.add) message+= "- Thêm mới: " + data.add + " số\n"
                        if (data.update) message+= "- Cập nhật: " + data.update + " số\n"
                        if (data.error) message+= "- Lỗi: " + data.error + " số\n"
                        if (data.nothing) message+= "- Không làm gì: " + data.nothing + " số\n"
                        alert(message);
                    },
                    error: function() {
                        $body.removeClass("loading");
...
function add_sim_ajax() {
    ...
    if (!isset($_POST['sim']) || !isset($_POST['agency']) || !isset($_POST['gia'])) {
    ...
    foreach ($simArr as $key => $simso) {
        ...
        $existsSimId = sim_exists($slug);
        if ($existsSimId) {
            if ($update) {
                update_post_meta($existsSimId, 'price', $sgia);
                wp_set_post_terms($existsSimId, [$agency], 'agency');
                $mySim = array(
                    'ID'         => $existsSimId,
                    'post_title' => $sso
                );
                wp_update_post($mySim);
                $response['update'] = $response['update'] + 1;
            } else {
                $response['nothing'] = $response['nothing'] + 1;
            }
        } else {
            $mySim = array(
                'post_title' => $sso,
            ...
            $simId = wp_insert_post($mySim);
            update_post_meta($simId, 'price', $sgia);
            wp_set_post_terms($simId, [$agency], 'agency');
            $response['add'] = $response['add'] + 1;
        }
    }
    echo json_encode($response);
    exit;
            ...
            <tbody>
                ...
                <tr class="alternate iedit">
                    ...
                    <td colspan="2" style="padding:20px 12px;">
                        ...
                        <label><input type="radio" name="update_action" value="3" required> Xóa số khỏi website</label>
...
function update_sim_ajax() {
    ...
    switch ($update_action) {
        case 1:
            $query = "UPDATE $wpdb->postmeta pm
                LEFT JOIN $wpdb->posts p
                ON p.ID = pm.post_id
                SET pm.meta_value = NULL
                WHERE pm.meta_key = 'price'
                AND p.post_name in $stringSo";
            break;
    ...
    if ($queryPm) $wpdb->query($queryPm);
    if ($query) {
        $countAffected = $wpdb->query($query);
        $type = ($update_action == 3) ? 'delete' : 'update';
        $response[$type] = $countAffected;
    }
    $wpdb->flush();
    echo json_encode($response);
    exit;
...
function getGia($simId) {
    $gia = get_post_meta(get_the_ID(), 'price', true);
    if ($gia == '') return 'Số đã bán';
